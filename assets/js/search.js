(function () {
  ...
  var ARROW_L =
    '<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M6.52341 9.16689L10.9934 4.69689L9.81508 3.51855L3.33342 10.0002L9.81508 16.4819L10.9934 15.3036L6.52341 10.8336H16.6667V9.16689H6.52341Z" fill="#161B25"/></svg>';
  var ARROW_R =
    '<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M13.4766 9.16689L9.00658 4.69689L10.1849 3.51855L16.6666 10.0002L10.1849 16.4819L9.00658 15.3036L13.4766 10.8336H3.33325V9.16689H13.4766Z" fill="#161B25"/></svg>';
    ...
    posts.forEach(function (post) {
      ...
      if (post.feature_image) {
        imgHtml =
          '<a href="' +
          escapeHtml(postUrl) +
          '" class="search-result-img" tabindex="-1" aria-hidden="true">' +
          '<img src="' +
          escapeHtml(post.feature_image) +
          '" alt="' +
      ...
      if (post.primary_tag) {
        tagHtml =
          ...
          escapeHtml(safeUrl(post.primary_tag.url)) +
          '">' +
      ...
      if (excerptText) {
        var trimmed =
          excerptText.length > 160
            ...
            : excerptText;
        excerptHtml =
          '<p class="search-result-excerpt">' + escapeHtml(trimmed) + "</p>";
      }
      ...
      if (post.primary_author) {
        authorHtml =
          '<a href="' +
          escapeHtml(safeUrl(post.primary_author.url)) +
  ...
  function goToPage(page, scroll) {
    ...
    if (!filteredPosts.length) {
      resultsContainer.innerHTML =
        '<p class="search-no-results">No posts found for \u201c<strong>' +
        escapeHtml(query) +
        "</strong>\u201d.</p>";
  ...
  function loadAllPosts() {
    ...
    fetch(url)
      ...
      .catch(function () {
        resultsContainer.innerHTML =
          '<p class="search-error">Something went wrong. Please try again.</p>';
      });
  }
  ...
  function setTagFilter(slug) {
    currentTag = slug;
    document
      .querySelectorAll("#search-tag-filters .filter-btn")
      .forEach(function (b) {
  ...
  function filterPosts(query) {
    ...
    var searchFiltered = !q
      ? allPosts
      : allPosts.filter(function (post) {
          return (
            (post.title && post.title.toLowerCase().indexOf(q) !== -1) ||
            (post.excerpt && post.excerpt.toLowerCase().indexOf(q) !== -1) ||
            (post.custom_excerpt &&
              post.custom_excerpt.toLowerCase().indexOf(q) !== -1)
          );
        });
    ...
    filteredPosts = !currentTag
      ? searchFiltered
      : searchFiltered.filter(function (post) {
          return (post.tags || []).some(function (t) {
            return t.slug === currentTag;
          });
        });
  ...
  var debouncedFilter = debounce(function (query) {
    filterPosts(query);
  }, 400);
